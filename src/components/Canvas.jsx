...
export default function Canvas ({ dificuldade, onVoltarMenu }){
  ...
  const menuAbertoRef = useRef(false); // Referência para o estado do menu
  ...
  useEffect(() => {
    // Atualiza a referência sempre que o estado mudar
  ...
  useEffect(() => {
    ...
    const sketch = (p) => {
      ...
      const numBuracosNegros = dificuldade === 'dificil' ? 2 : 1; // Número de buracos negros
      ...
      p.mouseReleased = () => {
        console.log(menuAbertoRef.current)
        if (menuAbertoRef.current) return;
  ...
  return (
      <div className="relative w-full h-full">
        ...
        <div className="absolute top-4 left-4 z-10">
          <button
              onClick={toggleMenu}
              className="bg-purple-800 bg-opacity-70 text-white px-4 py-2 rounded-full shadow-lg hover:bg-purple-700 transition-colors"
          >
            {menuAberto ? "X" : "☰"}
          </button>
